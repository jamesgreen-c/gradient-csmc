...
"""
USE DYNAMIC RESAMPLING BECAUSE RESAMPLING IS DESTROYING LINEAGES

FOR EACH DIMENSION SAMPLE PARTICLES FROM THE PRIOR DYNAMICS UP UNTIL ITS OBSERVATION (LIKELIHOOD).
THEN WEIGHT, RESAMPLE AND USE THE MONTE CARLO APPROXIMATION (DELTA) DISTRIBUTION AS THE POSTERIOR. 
COMPARE THIS TO THE RESULTS FROM THIS MODEL
"""
...
def get_data(key, dim, sigma_x, sigma_y, phi: float = 0.9):
    dt = 1.0 / dim
    T = dim
    ...
    eps_ys = sigma_y * jax.random.normal(key_y, (T,))           # small scalar obs noise
...
def log_potential(x, y, ind, sigma):
    ind = jnp.asarray(ind, dtype=jnp.int32)   # <--- add this
    val = norm.logpdf(y, loc=x[ind], scale=sigma)
    return jnp.sum(val)


def log_likelihood(x, y, ind, sigma):
    return jnp.sum(log_potential(x, y, ind, sigma))

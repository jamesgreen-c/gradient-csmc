...
def one_experiment(key):
    ...
    kernel, init, adaptation_loop, experiment_loop = kernel_type.kernel_maker(ys, inds, args.D, SIGMA, N=args.N,
    ...
    csmc_kernel, csmc_init, *_ = get_csmc_kernel(ys, inds, args.D, SIGMA, N=args.N, resampling_func=resampling_fn,
    ...
    energy = log_pdf(samples, ys, inds, SIGMA)
    if args.M > 1:
    ...
    return (means_here, std_devs_here, samples_ess, final_pct,
            ...
            samples_acf, esjd_vals, traces, inds)
...
for k, key_k in enumerate(EXPERIMENT_KEYS):
    ...
    (means_k, std_k, ess_k, final_pct_k,
     ...
     esjd_vals_k, traces_k, inds_k) = one_experiment(key_k)
    # print(ess_k.shape)
    toc = time.time()
...
np.savez_compressed(datapath, ess=ess_all, final_pct=final_pct_all, delta=adapted_delta_all,
                    # sampling_time=sampling_time_all, energy=energy_all,
                    means=means_all,
                    # std_devs=std_devs_all,
                    true_xs=true_xs_all, ys=ys_all,
                    init_xs=init_xs_all,
                    # iacf_all=acf_all,
                    # esjd_all=esjd_all, 

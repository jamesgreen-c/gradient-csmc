...
def get_csmc_kernel(m0, ys, inds, dx, phi, chol_P0, chol_Q, sigma_y, N, style="bootstrap", **kwargs):
...
def get_rw_csmc_kernel(m0, ys, inds, dx, phi, chol_P0, chol_Q, sigma_y, N, **kwargs):
    kwargs.pop("style")
    dt = 1 / dx
    A = jnp.exp(-phi * dt)
    ...
    def Gamma_0(x):
        return log_potential(x, ys[0], inds[0], sigma_y) + mvn_logpdf(x, m0, None, chol_inv=_chol_P0_inv, constant=False)
    ...
    def Gamma_t(x_t_m_1, x_t, _params):
        x_pred = A * x_t_m_1
        return log_potential(x_t, _params[0], _params[1], sigma_y) + mvn_logpdf(x_t, x_pred, None, chol_inv=_chol_Q_inv, constant=False)

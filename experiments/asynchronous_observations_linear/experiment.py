...
from experiments.asynchronous_observations_linear.kernels import KernelType, get_csmc_kernel
from experiments.asynchronous_observations_linear.model import get_data, log_pdf
...
def one_experiment(key):
    ...
    true_xs, m0, ys, chol_P0, chol_Q = get_data(data_key, SIGMA, args.D)
    ...
    return (means_here, std_devs_here, sample_ess, final_pct, esjd,
            # energy, 
            init_xs, true_xs, ys, adapted_delta, sample_bs)
...
ys_all = np.empty((args.K, args.D, args.D))
...
for k, key_k in enumerate(EXPERIMENT_KEYS):
    ...
    (means_k, std_k, ess_k, final_pct_k, esjd_vals_k,
     # energy_k,
     init_xs_k, true_xs_k, ys_k, adapted_delta_k, Bs_k) = one_experiment(key_k)
    toc = time.time()
...
np.savez_compressed(datapath, 
                    ...
                    delta=adapted_delta_all, Bs=Bs_all)
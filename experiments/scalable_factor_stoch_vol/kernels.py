...
from experiments.scalable_factor_stoch_vol.model import log_likelihood, log_potential, log_pdf
from experiments.scalable_factor_stoch_vol.utils import _check_param_shapes
...
def get_tp_kernel(ys, m0, P0, Q, F, b, B, V, N=1, **_kwargs):
    raise(NotImplementedError)


def get_mala_kernel(ys, m0, P0, Q, F, b, B, V, N=1, style="marginal", **_kwargs):
    raise(NotImplementedError)


def get_csmc_kernel(ys, m0, P0, Q, F, b, B, V, N, style="bootstrap", **kwargs):
    """
    ...
    :param B: Factor loading matrix
    :param V: Process noise for observations
    ...
    if style == "bootstrap":
        ...
        Gamma_0 = lambda x: log_potential(x, ys[0], B, V) + M0_logpdf(x)
        Gamma_t = lambda x_t_m_1, x_t, y: log_potential(x_t, y, B, V) + Mt_logpdf(x_t_m_1, x_t, None)
...
def get_tp_csmc_kernel(ys, m0, P0, Q, F, b, B, V, N, style="marginal", stop_gradient=False, **kwargs):
    """
    ...
    :param B: Factor loading matrix
    :param V: Process noise covariances for observations
    :param kwargs: Additional keyword arguments
    """

    def r0(x):
        if stop_gradient:
            return log_potential(jax.lax.stop_gradient(x), ys[0], B, V)
        return log_potential(x, ys[0], B, V)

    def rt(_, x, y):
        if stop_gradient:
            return log_potential(jax.lax.stop_gradient(x), y, B, V)
        return log_potential(x, y, B, V)
...
def get_mala_csmc_kernel(ys, m0, P0, Q, F, b, B, V, N, style="marginal", **kwargs):
    """
    ...
    def Gamma_0(x):
        return log_potential(x, ys[0], B, V) + mvn_logpdf(x, m0, None, chol_inv=chol_P0_inv, constant=False)

    @partial(jnp.vectorize, signature='(d),(d),(n)->()')
    def Gamma_t(x_t_m_1, x_t, y):
        x_pred = x_t_m_1 @ F.T + b
        return log_potential(x_t, y, B, V) + mvn_logpdf(x_t, x_pred, None, chol_inv=chol_Q_inv, constant=False)
...
def get_rw_csmc_kernel(ys, m0, P0, Q, F, b, B, V, N, **kwargs):
    """
